...
func (c *controller) NewImportOperation() *framework.PathOperation {
	...
	return &framework.PathOperation{
		...
		Description: "Imports a wallet given a private key, storing it in the Vault and computing its public key and address",
...
func (c *controller) importHandler() framework.OperationFunc {
	return func(ctx context.Context, req *logical.Request, data *framework.FieldData) (*logical.Response, error) {
		...
		account, err := c.useCases.CreateWallet().WithStorage(req.Storage).Execute(ctx, namespace, privateKeyString)
		if err != nil {
